...
def pay(xml, secret):
    ...
    return _pay(orderXML=order_xml, sha512=order_sha)


def _pay(**kwargs):
    """Process payment internal.

    Use this method to pass custom arguments for the POST request. You can pass parameters orderXML (encoded with
    base64) and sha512 directly bypassing XML conversion to string in :func:`PyCardPay.api.pay` but you must be sure
    that the data have not been modified by the user when submitting the form.

    :param \*\*kwargs: HTTP post payload
    :raises: :class:`PyCardPay.exceptions.XMLParsingError` if response contains unknown xml structure.
    :returns: dict -- see :func:`PyCardPay.api.pay` for description
    """
    r = make_http_request(url_pay, method='post', **kwargs)
    print r
    ...
    raise XMLParsingError(u'Unknown XML response. Root tag contains no order nor redirect: {}'.format(r), r)


def finish_3ds(md, pa_res):
    ...
    :type md: str|unicode
    :param pa_res: Response code from bank after authorization.
    :type pa_res: str|unicode
    :raises: :class:`PyCardPay.exceptions.XMLParsingError` if response contains unknown xml structure.
    :returns: dict -- see :func:`PyCardPay.api.pay` for description.
    """
    return _pay(MD=md, PaRes=pa_res)